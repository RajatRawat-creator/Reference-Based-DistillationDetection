#!/usr/bin/env python3
# Single-Target Min-K% Prob Reference-Normalized Loss MIA (Server)
# Score = mink_target - mink_ref
# where mink = mean of bottom k% of per-token log-probs
...
FILES_MAP = {
    "Llama-3.3-70B-Instruct (s1)": "Llama-3.3-70B-Instruct-FP8__s1k__chat__vllm (4).jsonl",
    "Llama-3.3-70B-Instruct (OMI)": "Llama-3.3-70B-Instruct-FP8__openmath_jsonl__chat__vllm (5).jsonl",
 
    "Gemma-3-27B-it (s1)": "gemma-3-27b-it__s1k__chat__vllm.jsonl",
    "Gemma-3-27B-it (OMI)": "gemma-3-27b-it__openmath_jsonl__chat__vllm (1).jsonl",
 
    "Claude-3.5-Sonnet (s1)": "Claude-Sonnet-3.5_s1(200).jsonl",
    "Claude-3.5-Sonnet (OMI)": "Claude-Sonnet-3.5_omi(200).jsonl",
 
    "Claude Opus 4.5 (s1, Trace Only)": "Claude-Opus-4.5_s1(200).jsonl",
    "Claude Opus 4.5 (OMI, Trace Only)": "Claude-Opus-4.5_omi(200).jsonl",

    "Claude Opus 4.6 (s1, Trace Only)": "Claude-Opus-4.6_s1(200).jsonl",
    "Claude Opus 4.6 (OMI, Trace Only)": "Claude-Opus-4.6_omi(200).jsonl",
 
    "GPT-OSS-120B (s1)": "gpt-oss-120b__s1k__chat__vllm (5).jsonl",
    "GPT-OSS-120B (OMI)": "gpt-oss-120b__openmath_jsonl__chat__vllm (2).jsonl",
    ...
    "DeepSeek R1 (s1, Trace Only)": "R1_s1(200)_reasoningonly.jsonl",
 
    "DeepSeek R1 (OMI, Response Only)": "R1_omi(200)_responseonly.jsonl",
    "DeepSeek R1 (OMI, Trace Only)": "R1_omi(200)_reasoningonly.jsonl",
 
    "Qwen-3-235B-A22B-Thinking-2507 (OMI, Trace Only)": "Qwen-3-235B_omi(200)_reasoningonly.jsonl",
    "Qwen-3-235B-A22B-Thinking-2507 (s1, Trace Only)": "Qwen-3-235B_s1(200)_reasoningonly.jsonl",

    "QwQ-32B (OMI, Trace Only)": "QwQ-32B-omi(200).jsonl",
    "QwQ-32B (s1, Trace Only)": "QwQ-32B-s1(200).jsonl",
}
...
class ModelWrapper:
    """
    Scoring logic with both get_loss (unchanged) and get_mink_score (new).
    """
    def __init__(
        self,
        model,
    ...
    def get_mink_score(self, document: str):
        """
        Min-K% Prob score: mean of the bottom k% of per-token log-probs
        over the answer tokens, using the same windowing as get_loss.
        Returns a negative number (more negative = less likely).
        """
        prompt_ids, answer_ids = self._tokenize_prompt_answer(document)
        if prompt_ids is None:
            return None
        ...
        for i in range(0, len(answer_ids), stride):
            ...
            input_ids_list = merged_prompt_ids + prev_answer_ids + new_answer_ids
            n_prefix = len(merged_prompt_ids) + len(prev_answer_ids)
            ...
            # For token at position t, logits[:,t,:] predicts token t+1
            # We want log-prob of each answer token given its context
            logits = out.logits  # [1, seq_len, V]
            log_probs = F.log_softmax(logits, dim=-1)  # [1, seq_len, V]
 
            # The answer tokens to score are at positions n_prefix .. n_prefix+trg_len-1
            # The logit that predicts position p is at position p-1
            for j in range(trg_len):
                pos = n_prefix + j          # position of the answer token in input
                pred_pos = pos - 1          # logit position that predicts it
                if pred_pos < 0:
                    continue
                token_id = new_answer_ids[j]
                lp = float(log_probs[0, pred_pos, token_id].cpu())
                if np.isfinite(lp):
                    all_token_logprobs.append(lp)
 
        if len(all_token_logprobs) == 0:
            return None
 
        # Take bottom k% (lowest log-probs = hardest tokens)
        all_token_logprobs.sort()
        k_count = max(1, int(len(all_token_logprobs) * self.mink_k))
        min_k_probs = all_token_logprobs[:k_count]
        return float(np.mean(min_k_probs))
...
def main():
    ...
    ap.add_argument("--mink_k", type=float, default=0.2, help="Fraction k for Min-K%% Prob (default 0.2)")
    ...
    print("\n==================================================")
    print(f"📥 PHASE 1: PRE-COMPUTING REFERENCE Min-K SCORES ({args.ref_model})")
    print("==================================================")
    ...
    print(f"[INFO] mink_k = {args.mink_k}")
 
    ref_scores_cache = {k: [] for k in datasets.keys()}
    for ds_label, samples in datasets.items():
        for text in samples:
            ref_scores_cache[ds_label].append(ref_wrap.get_mink_score(text))
    ...
    try:
        ...
        results = {"MinK Ref-Norm": {ds: [] for ds in datasets}}
        fail_counts = {ds: 0 for ds in datasets}
 
        for ds_label, samples in datasets.items():
            ref_list = ref_scores_cache[ds_label]
            for i, text in enumerate(samples):
                ref_score = ref_list[i]
                if ref_score is None:
                    fail_counts[ds_label] += 1
                    continue
 
                target_score = wrap.get_mink_score(text)
                if target_score is None or not np.isfinite(target_score):
                    fail_counts[ds_label] += 1
                    continue
 
                # Reference-normalized Min-K score
                results["MinK Ref-Norm"][ds_label].append(target_score - ref_score)
        ...
        probs_dict, mu, sigma = scores_to_probabilities(results["MinK Ref-Norm"])
        ...
        if has_data:
            plt.title(f"{pretty_label(target_name)}\nMin-K% Ref-Norm Attack CDF (k={args.mink_k})", fontsize=16)
            ...
            plt.tight_layout()
            fig_path = Path(args.out_dir) / f"{safe_name(target_name)}__mink_ref_attack_cdf.png"
